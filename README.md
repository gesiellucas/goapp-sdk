...
try {
  const saida = {
    ...
    CODCONTR: 123, // Código Livre do Contrato (codigoFree/externalCode) ou ID UUID do contrato
...
| **CODCONTR** | String/Number| `contractId` (UUID) | Resolvido para o ID UUID do contrato (busca por `id` ou `externalCode`). |
...
| *Todos os campos* | JSON | `metadata` (jsonb) | **Todos os campos** recebidos pelo SDK (incluindo `CODEMP`, `ANO` e `MES` extraídos) são salvos na coluna `metadata`. |
